...
run_service() {
	...
	echo
	runsig.sh -lat $lat -lon $lon -txh $hei -f $freq -erp $erp -rt 39 -rel 50 -res 600 -R 250 -color /opt/Signal-Server/color/blue.scf -o $freq-service | genkmz.sh 
	echo
}
...
run_interference() {
	...
	echo
	runsig.sh -lat $lat -lon $lon -txh $hei -f $freq -erp $erp -rt 21 -rel 10 -res 600 -R 250 -color /opt/Signal-Server/color/green.scf -o $freq-interference | genkmz.sh
	echo
}
...
run_adjacent () {
	...
	echo
	runsig.sh -lat $lat -lon $lon -txh $hei -f $freq -erp $erp -rt 41 -rel 10 -res 600 -R 250 -color /opt/Signal-Server/color/magenta.scf -o $freq-adjacent | genkmz.sh
	echo
}
